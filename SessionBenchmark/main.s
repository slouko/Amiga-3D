...
; --- Optional benchmark mode via CLI argument ---
_start:
        move.l  4.w,a6
        jsr     -294(a6)          ; FindTask(0)
        move.l  d0,a1
        move.l  172(a1),a2        ; pr_CLI
        beq.s   .run_normal       ; If not CLI, run normal

        move.l  46(a2),a5         ; CLI->cli_CommandLine
...
; --- Find argument in command line (a5=cmdline, a0=searchstr, returns d0=found) ---
find_arg:
        move.l  a5,a1
.nextc:
        tst.b   (a1)
...
; files for easier access and better readability
    ...
    INCLUDE "bench.s"   ; Benchmarking code for measuring FPS
    even
